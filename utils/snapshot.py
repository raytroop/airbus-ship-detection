'''
https://github.com/keras-team/keras-contrib/blob/46fcdb9384b3bc9399c651b2b43640aa54098e64/keras_contrib/callbacks/snapshot.py#L1-L91
'''
...
class SnapshotModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch != 0 and (epoch + 1) % self.check == 0:
            filepath = self.fn_prefix + '_{:1d}.h5'.format((epoch + 1) // self.check)
...
class SnapshotCallbackBuilder:
    ...
    def get_callbacks(self, save_dir, model_prefix='mask_rcnn_airbus'):
        """
        ...
        callback_list = [ModelCheckpoint(os.path.join(save_dir, '{:s}_Best.h5'.format(model_prefix)),
                                         save_best_only=True, save_weights_only=True),
                        LearningRateScheduler(schedule=self._cosine_anneal_schedule),
                        SnapshotModelCheckpoint(self.T, self.M, fn_prefix=os.path.join(save_dir, model_prefix))]
